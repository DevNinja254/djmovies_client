       ...
       { submitted ? <p className='fixed top-2 right-2 bg-green-600 text-white font-bold textMidSm p-2 rounded-sm z-20'>message sent successful</p> : null}
        <div className='sm:w-5/6 m-auto'>
            <div className='mx-4 text-center my-3 pt-6'>
                ...
                <p className='text-gray-800 opacity-90   text-center text-sm'>Lights, camera, connection! Reach out with your questions, feedback, or casting calls. We're all ears behind the scenes. </p>
            </div>
            ...
            <div className='lg:w-4/5 2xl:p-10 lg:m-auto lg:bg-gray-50 lg:p-7 lg:mb-3 rounded-md'>
                <div className='m-4 my-10 text-center borderGrayB pb-10 lg:my-16'>
                    ...
                    <p className='text-sm  md:text-md'>Contact us for all your questions and opinions, or you can solve your problems in a shorter time with our contact offices.</p>
                </div>
                <form  className='mx-4 my-5 pt-3 text-gray-600' onSubmit={handleSubmit}>
                    <div className='lg:grid grid-cols-2 gap-4 '>
                        <div>
                            <label className="block text-sm py-2" htmlFor="Name">Name</label>
                            <input className='block bg-gray-500 w-full py-2 rounded-md bg-opacity-10 outline-none text-gray-700 px-2' type="text" name='name' value={formData.name} onChange={handleChange} disabled={submitting}/>
                            </div>
                        <div>
                            <label className="block text-sm py-2" htmlFor="email">Email <span className='text-red-500'>*</span></label>
                            <input className='block bg-gray-500 w-full py-2 rounded-md bg-opacity-10 outline-none text-gray-700 px-2' type="email" required name="email" value={formData.email} onChange={handleChange} disabled={submitting}/>
                        </div>
                    </div>
                    <label className="block text-sm py-2" htmlFor="number">Phone Number</label>
                    <input className='block bg-gray-500 w-full py-2 rounded-md bg-opacity-10 outline-none text-gray-700 px-2' type="number" required name="phone_number" value={formData.phone_number} onChange={handleChange} disabled={submitting}/>
                    <label className="block text-sm py-2" htmlFor="message">Your Message <span className='text-red-500'>*</span></label>
                    <textarea name="message" required id="messade" rows={5} className='block bg-gray-500 w-full py-2 rounded-md bg-opacity-10 outline-none text-gray-700 px-2' value={formData.message} onChange={handleChange} disabled={submitting}></textarea>
